...
def fig_3a_flowchart():
    ...
    font_kw = dict(ha="center", va="center", fontsize=9, fontweight="bold")
    ...
    # Phase labels
    ax.text(3.5, 7.6, "Phase 1: Synthetic Validation", fontsize=12,
            fontweight="bold", color="#2C3E50", ha="center")
    ax.text(10.5, 7.6, "Phase 2: Real-World Hierarchical", fontsize=12,
            fontweight="bold", color="#2C3E50", ha="center")
    ...
    # Cross-phase arrow
    ax.annotate("", xy=(8.5, 4.9), xytext=(5.5, 4.3),
                arrowprops=dict(arrowstyle="-|>", color="#E67E22", lw=2,
                                linestyle="dashed"))
    ax.text(6.8, 4.9, "informs\ndesign", fontsize=7, color="#E67E22",
...
def fig_3b_confusion():
    ...
    cm_path = os.path.join(OUTPUT_DIR, "ctx_baseline", "real", "hierarquico",
    ...
    fig.suptitle("Confusion Matrices — Direct vs Hierarchical Classification",
...
def fig_3d_evolution():
    ...
    color_labels = {"synth": "Synthetic (test set)", "real": "Real (LOGO CV)",
                    "holdout": "Real (holdout)"}
    x = np.arange(len(stages))
    bars = ax.bar(x, [s[1] for s in stages],
                  color=[colors[s[2]] for s in stages],
    ...
    ax.text(1.5, 88, "Phase 1: Synthetic", ha="center", fontsize=10,
            color="#2C3E50", fontweight="bold")
    ax.text(5, 88, "Phase 2: Real", ha="center", fontsize=10,
            color="#2C3E50", fontweight="bold")
    ...
    ax.legend(handles=handles, loc="center left", framealpha=0.9,
              bbox_to_anchor=(0.0, 0.55))

    ax.set_title("Progressive Accuracy Evolution (Stage 0 → 6)", fontsize=13,
                 fontweight="bold")
...
def fig_3e_label_quality():
    ...
    csv_path = os.path.join(OUTPUT_DIR, "step05_pro", "real", "label_analysis",
    ...
    # Overall histogram
    ax1.hist(df["quality_score"], bins=50, color="#3498DB", edgecolor="white",
             alpha=0.8)
    mean_q = df["quality_score"].mean()
    median_q = df["quality_score"].median()
    ax1.axvline(mean_q, color="#E74C3C", linestyle="--", lw=2,
                label=f"Mean: {mean_q:.3f}")
    ax1.axvline(median_q, color="#F39C12", linestyle="-.", lw=2,
                label=f"Median: {median_q:.3f}")
    ax1.axvline(0.5, color="#27AE60", linestyle=":", lw=2,
                label="Issue threshold: 0.5")
    ...
    for cls in ["MCAR", "MAR", "MNAR"]:
        ...
        ax2.hist(subset, bins=30, alpha=0.5, color=class_colors[cls],
                 label=f"{cls} (μ={subset.mean():.3f})", edgecolor="white")
    ax2.set_xlabel("Label Quality Score")
    ax2.set_ylabel("Count")
    ax2.set_title("Distribution by Class")
    ax2.legend(fontsize=9)

    fig.suptitle("Cleanlab Label Quality Score Distribution",
                 fontsize=13, fontweight="bold")
...
def fig_3f_confident_joint():
    ...
    sns.heatmap(cj, annot=True, fmt="d", cmap="YlOrRd", ax=ax,
    ...
    ax.set_title("Confident Joint Matrix (n=1,132)", fontsize=13,
                 fontweight="bold")

    # Annotate key finding
    ax.text(1.5, -0.3, "MAR absorbs 109 MCAR + 215 MNAR samples",
...
def fig_3g_gap():
    ...
    bars1 = ax.bar(x - width/2, synthetic, width, label="Synthetic",
                   color="#3498DB", edgecolor="white")
    bars2 = ax.bar(x + width/2, real, width, label="Real (LOGO CV)",
                   color="#E74C3C", edgecolor="white")

    # Gap annotations
    for i, gap in enumerate(gaps):
        ax.annotate(f"{gap}pp", xy=(i + width/2, real[i]),
    ...
    ax.set_title("Performance Gap: Synthetic vs Real Data by Class",
                 fontsize=13, fontweight="bold")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def fig_3h_cohens_d():
    ...
    # Reference lines
    for val, label, ls in [(0.2, "Small", ":"), (0.5, "Medium", "--"),
                           (0.8, "Large", "-.")]:
        ax.axvline(val, color="#95A5A6", linestyle=ls, lw=1, alpha=0.7)
        ax.text(val, len(names) - 0.3, label, fontsize=7, color="#95A5A6",
                ha="center")
    ...
    handles = [mpatches.Patch(color="#3498DB", label="CAAFE (pure Python)"),
               mpatches.Patch(color="#E74C3C", label="LLM (API calls)")]
    ax.legend(handles=handles, loc="lower right", fontsize=9)

    ax.set_title("Effect Size: CAAFE vs LLM Features", fontsize=13,
                 fontweight="bold")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def fig_3i_logo_cv():
    ...
    csv_path = os.path.join(OUTPUT_DIR, "step05_pro", "real",
                            "hierarquico_v3plus", "cv_logo_v3plus.csv")
    df = pd.read_csv(csv_path)
    ...
    bars = ax.bar(x, [m * 100 for m in means], width=0.7, color=colors,
                  edgecolor="white", yerr=[s * 100 for s in stds],
                  capsize=3, error_kw={"lw": 1, "color": "#555"})

    for i, (m, s) in enumerate(zip(means, stds)):
        ax.text(i, m * 100 + s * 100 + 1.5, f"{m*100:.1f}%",
    ...
    handles = [mpatches.Patch(color="#27AE60", label="NaiveBayes (best)"),
               mpatches.Patch(color="#E74C3C", label="XGBoost"),
               mpatches.Patch(color="#3498DB", label="Other models")]
    ax.legend(handles=handles, loc="upper right", fontsize=9)

    ax.set_title("LOGO Cross-Validation Accuracy — V3+ Soft 3-Zone Routing",
                 fontsize=13, fontweight="bold")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def copy_existing():
    ...
    copies = [
        (os.path.join(OUTPUT_DIR, "step05_pro", "real", "shap_analysis",
                      "shap_l1_vs_l2_comparison.png"),
         "shap_l1_vs_l2.png"),
        (os.path.join(OUTPUT_DIR, "step05_pro", "real", "shap_analysis",
                      "shap_direto_3way.png"),
         "shap_3way.png"),
        (os.path.join(OUTPUT_DIR, "step05_pro", "real", "shap_analysis",
                      "confusion_v3_hier.png"),
         "confusion_v3plus.png"),
        (os.path.join(OUTPUT_DIR, "step05_pro", "real", "label_analysis",
