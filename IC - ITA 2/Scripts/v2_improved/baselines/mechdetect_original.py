    ...
    for fname in tqdm(files, desc=f"  {class_name}", leave=False):
        ...
        try:
            ...
            results.append({
                ...
                "pred_label": "MCAR",
                "pred_code": 0,
                "auc_complete": 0.5, "auc_shuffled": 0.5, "auc_excluded": 0.5,
                "delta_complete_shuffled": 0.0, "delta_complete_excluded": 0.0,
                "mwu_pvalue": 1.0,
            })
...
report = classification_report(y_true, y_pred, target_names=["MCAR", "MAR", "MNAR"],
...
for cls in ["MCAR", "MAR", "MNAR"]:
    ...
    print(f"     {cls}: auc_c={sub['auc_complete'].mean():.3f} "
...
for ax, metric in zip(axes, ["auc_complete", "auc_excluded", "delta_complete_excluded"]):
